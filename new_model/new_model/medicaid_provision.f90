subroutine medicaid_provision()
    ...
    1       R_max=(30.0_sp+p_fc*2.0_sp*365.0_sp*2.0_sp)*g
            call solve_intratemporal(p_fc,R_max,h_l,0.0_sp,u_g,l_bar(h_l),c_bar(h_l))
    ...
    2       R_g=(R_max+R_min)/2.0_sp
            call solve_intratemporal(p_fc,R_g,h_l,0.0_sp,u_g,l_bar(h_l),c_bar(h_l))
        ...
        do h_l=1,clusters; do f_l=1,f_t
            if (h_l==1) then
                ...
                u_bar(h_l,f_l)=c_bar(h_l)**(1.0_sp-sigma)/(1.0_sp-sigma)+mu(h_l)*(l_bar(h_l)+omega*l_ic(f_l,h_l)+kappa_h(h_l))**(1.0_sp-nu)/(1.0_sp-nu)
            end if
        end do;end do
