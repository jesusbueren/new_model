    ...
    beq_k2=0.0_sp
     do ts_l2=1,nzz;do ps_l2=1,nzz;do h_l2=1,clusters+1; do nh_l2=1,2
        if (h_l2<clusters+1) then
            ...
            x2=max(coh_grid(k_l2)-m_exp_all(t_l,ge_l,int(real(i_l+1)/2.0_sp),h_l,ps_l2,ts_l2)/2.0_sp-real(nh_l2-1)*p_nh(h_l2),coh_min)
        end if
            ...
            if (coh_grid(pos_x2)==x2) then
                ECV_k2=ECV_k2+pr_ep(ts_l2,1)*pr_pxi(ps_l,ps_l2)*H_av(h_l,h_l2,t_l,int(real(i_l+1)/2.0_sp),ge_l)*pr_nh(f_l,h_l,nh_l,nh_l2)*V(pos_x2,h_l2,ps_l2,nh_l2,2)
                beq_k2=beq_k2+pr_ep(ts_l2,1)*pr_pxi(ps_l,ps_l2)*H_av(h_l,h_l2,t_l,int(real(i_l+1)/2.0_sp),ge_l)*pr_nh(f_l,h_l,nh_l,nh_l2)*beq_aux(pos_x2,h_l2,ps_l2,nh_l2,2)
            else
                ECV_k2=ECV_k2+pr_ep(ts_l2,1)*pr_pxi(ps_l,ps_l2)*H_av(h_l,h_l2,t_l,int(real(i_l+1)/2.0_sp),ge_l)*pr_nh(f_l,h_l,nh_l,nh_l2)*&
                (alpha*V(pos_x2,h_l2,ps_l2,nh_l2,2)+(1.0_sp-alpha)*V(pos_x2+1,h_l2,ps_l2,nh_l2,2))
                beq_k2=beq_k2+pr_ep(ts_l2,1)*pr_pxi(ps_l,ps_l2)*H_av(h_l,h_l2,t_l,int(real(i_l+1)/2.0_sp),ge_l)*pr_nh(f_l,h_l,nh_l,nh_l2)*&
