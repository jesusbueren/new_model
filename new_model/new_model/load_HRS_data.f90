    ...
    real(SP),dimension(clusters*2+2,obs,indv)::data_h
    real(SP),dimension(indv,obs,clusters*2+2):: pr_h_i
    ...
    pr_h_i=reshape(data_h, (/indv, obs,clusters*2+2/), order = (/ 3,2, 1 /))
    s_h_i=pr_h_i(:,:,1:clusters)
    f_h_i=pr_h_i(:,:,clusters+1:2*clusters)
    dead_i=pr_h_i(:,:,2*clusters+1)
    nhmliv_i=pr_h_i(:,:,2*clusters+2)
