subroutine solve_model(a_policy,g_policy,lfc_x,u_x,beq100_policy)
    ...
    implicit none
    real(SP),dimension(nkk,clusters,nzz,L_gender,L_PI2,f_t,2,generations),intent(out)::beq100_policy
    integer,dimension(nkk,clusters,nzz,L_gender,L_PI2,f_t,2,generations),intent(out)::a_policy,g_policy  !Policy for assets
    real(SP),dimension(nkk,clusters,f_t,L_PI2,2),intent(out)::u_x
    real(SP),dimension(nkk,clusters,f_t,L_PI2,2),intent(out)::lfc_x
    real(SP),dimension(nkk,clusters+1,nzz,2,2)::V,beq_aux
    real(SP),dimension(nkk,clusters,f_t,L_PI2,2)::c_x
    ...
    do i_l=1,L_PI2;do f_l=1,f_t;do h_l=1,clusters;do nh_l=1,2
        do x_l=1,nkk;
        if (x_l==1) then
    ...
    !$OMP PARALLEL default(none) private(i_l,t_l,ps_l,h_l,x_l,k2_l_min,nh_l,V_k2,ECV_k2_no_beq,V_MD,V_wo_MD,k2_wo_MD,k_l2,ge_l,f_l,V,beq_aux,beq_wo_md,beq_md,pause_k) shared(g_policy,beq100_policy,a_policy,u_x,u_bar,coh_grid,delta,lambda,sigma,sigma_beq,omega,beta,V_70)
    !$OMP  DO collapse(3)
    do ge_l=1,L_gender;
    do f_l=1,f_t 
    do i_l=1,L_PI2
    ...
    do x_l=1,nkk
        !Std VFI
        if (x_l>2) then
            k2_l_min=a_policy(x_l-1,h_l,ps_l,ge_l,i_l,f_l,nh_l,t_l) 
        else
            k2_l_min=1
        end if
        ...
        !Value of Medicaid
        k_l2=1
        call ECV_V_k_l2(u_x(x_l-k_l2+1,h_l,f_l,i_l,nh_l),V,ge_l,i_l,f_l,t_l,h_l,ps_l,x_l,k_l2,nh_l,V_k2,ECV_k2_no_beq,beq_aux,beq_MD)
        V_MD=u_bar(h_l,f_l,nh_l)+beta*ECV_k2_no_beq
        !if (isnan(V_MD)) then
        !    print*,''
        ...
        if (V_MD>=V_wo_MD) then
            V(x_l,h_l,ps_l,nh_l,1)=V_MD
            a_policy(x_l,h_l,ps_l,ge_l,i_l,f_l,nh_l,t_l)=1
            g_policy(x_l,h_l,ps_l,ge_l,i_l,f_l,nh_l,t_l)=1
            beq100_policy(x_l,h_l,ps_l,ge_l,i_l,f_l,nh_l,t_l)=beq_MD
        else
            V(x_l,h_l,ps_l,nh_l,1)=V_wo_MD
            a_policy(x_l,h_l,ps_l,ge_l,i_l,f_l,nh_l,t_l)=k2_wo_MD
            g_policy(x_l,h_l,ps_l,ge_l,i_l,f_l,nh_l,t_l)=0
            beq100_policy(x_l,h_l,ps_l,ge_l,i_l,f_l,nh_l,t_l)=beq_wo_md 
        end if
