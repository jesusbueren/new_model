subroutine simulate_model(a_policy,g_policy,lfc_x,beq100_policy, &
    ...
    implicit none
    real(SP),dimension(nkk,clusters,nzz,L_gender,L_PI2,f_t,2,generations),intent(in)::beq100_policy
    integer,dimension(nkk,clusters,nzz,L_gender,L_PI2,f_t,2,generations),intent(in)::a_policy,g_policy
        ...
        do i_l=1,indv
            ...
            do t_l=1,obs
                if (h_i(i_l,t_l,1) /= -9) then
                    ...
                    !Nursing home state
                    nh_l=nhmliv_i(i_l,t_l)+1
                    
                    !Cash on hand
                    x_it(t_l)=(1+r)*a_it(t_l)-m_exp_all(generation_i(i_l)+t_l-1,gender_i(i_l),PI_q_i2(i_l),h_i(i_l,t_l,1),xi_l2,ts_l2)+b(PI_q_i2(i_l),gender_i(i_l))-real(nh_l-1)*p_nh(h_i(i_l,t_l,1))
                    pos_x=int(max(min(x_it(t_l),coh_grid(nkk)),0.0_sp)/(coh_grid(2)-coh_grid(1))+1.00000001_sp)
                    !Draw on the discrete choice
                    call RANDOM_NUMBER(u)

                    if (u<g_policy(pos_x,h_i(i_l,t_l,1),xi_l,gender_i(i_l),PI_q_i2(i_l),f_l(1),nh_l,generation_i(i_l)+t_l-1)) then
                        g_it(t_l)=1.0_sp
                        k2_l=1
                    else
                        g_it(t_l)=0.0_sp
                        k2_l=a_policy(pos_x,h_i(i_l,t_l,1),xi_l,gender_i(i_l),PI_q_i2(i_l),f_l(1),nh_l,generation_i(i_l)+t_l-1)
                    end if
                    if (t_l<obs) then
                    ...
                    if (beq100(i_l,t_l)/=-9.0_sp) then
                        ...
                        beq100_ic(PI_q_i(i_l),f_l(2),counter_beq100(PI_q_i(i_l),f_l(2)))=beq100_policy(pos_x,h_i(i_l,t_l,1),xi_l,gender_i(i_l),PI_q_i2(i_l),f_l(1),nh_l,generation_i(i_l)+t_l-1)
                ...
                if (isnan(sum(moments_it(:,t_l,i_l)))) then
                    print*,beq100_it
                    print*,'problem in simulate model'
                    read*,pause_k
                end if
            end do
        end do
